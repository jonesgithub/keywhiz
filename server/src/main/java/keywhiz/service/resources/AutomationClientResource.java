...
public class AutomationClientResource {
  ...
  @Inject
  public AutomationClientResource(ClientJooqDao clientJooqDao, AclJooqDao aclJooqDao) {
  ...
  public Response findClient(
    ...
    List<ClientDetailResponse> clients = clientJooqDao.getClients().stream()
        .map(c -> ClientDetailResponse.fromClient(c, ImmutableList.copyOf(aclJooqDao.getGroupsFor(c)),
            ImmutableList.of()))
        .collect(toList());
